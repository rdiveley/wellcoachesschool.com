...
<cfinvoke component="utilities/XML-RPC"
  ...
  <cftry>
      ...
      <cfinclude template="showHideSurveyTally.cfm">
      	...
      	<cfquery name="insertResults" datasource="#DSN#" username="#application.DSNuName#" password="#application.dsnpword#" >
            ...
            (other,hours,lesson,lessonYear,email,classTitle,coachingSkills,businessDev,faculty,valuableSkills,summarize,awareOf,dateSubmitted,infusionSoftID)
            VALUES
            (
                 <cfqueryparam value="#URL.other#" cfsqltype="cf_sql_varchar"/>
                 <cfif addSurveyTally>
                	,<cfqueryparam value="#val(URL.hours)#" cfsqltype="cf_sql_float"/>
                 <cfelse>
                 	,<cfqueryparam value="0.0" cfsqltype="cf_sql_float"/>
                 </cfif>
                ,<cfqueryparam value="#URL.lesson#" cfsqltype="cf_sql_varchar"/>
                ,<cfqueryparam value="#val(URL.year)#" cfsqltype="cf_sql_varchar"/>
                ,<cfqueryparam value="#URL.email#" cfsqltype="cf_sql_varchar"/>
                ,<cfqueryparam value="#URL.classAttended#" cfsqltype="cf_sql_varchar"/>
                ,<cfqueryparam value="#URL.coachingSkills#" cfsqltype="cf_sql_varchar"/>
                ,<cfqueryparam value="#URL.businessDev#" cfsqltype="cf_sql_varchar"/> 
  ...
  <cffile action="write"
   ...
   <cffile action="readbinary" file="#expandPath('./temp/#uniqueFileName#')#" variable="readFile">
    ...
    <cfcatch type="any">
      <cfmail to="techsupport@wellcoaches.com" subject="Survey Gizmo 2 Error" from="#URL.email#">
      		#cfcatch.detail#
             <cfdump var="#URL#">
      </cfmail>
  </cfcatch>    
