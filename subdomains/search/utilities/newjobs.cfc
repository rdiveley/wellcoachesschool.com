<cfcomponent>
	...
	<cffunction name="GetJobNamebyInvoiceInstructions" access="remote" returntype="string" output="true">
  	...
  	<cffunction access="remote" name="GetHotJobs" output="true" returntype="query">
		...
		<cfquery name="GetHotJobs" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from jobs 
	...
	<cffunction access="remote" name="GetCurrentJobs" output="true" returntype="query">
  	...
  	<cffunction access="remote" name="GetJobDetails" output="true" returntype="query">
		...
		<cfquery name="GetJob" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from jobs where jobid=#arguments.jobid#
			#orderByStatement#
		</cfquery>
		<cfreturn GetJob>
  	</cffunction>
	...
	<cffunction access="remote" name="GetCategories" output="true" returntype="query">
  	...
  	<cffunction access="remote" name="GetJobsbyInvoiceInstructions" output="true" returntype="query">
		...
		<cfquery name="GetRecordsRet" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from jobs 
			<Cfif #Arguments.InvoiceInstructions# neq 0>
			<cfset InvoiceInstructions=#Arguments.InvoiceInstructions#>
			where InvoiceInstructions='#Trim(InvoiceInstructions)#'
			</cfif>
			order by JobName
		</cfquery>	
		<cfreturn GetRecordsRet>
  	...
  	<cffunction access="remote" name="GetCatalogItems" output="true" returntype="query">
		...
		<cfquery name="GetAllJobs" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from jobs order by jobname
		</cfquery>
		...
		<cfquery name="GetCatalogueItems" dbtype="query">
			...
			where Jobcategoryid='#arguments.Jobcategoryid#' 
	...
	<cffunction name="JobsAdmin" access="remote" returntype="string" output="true">
		...
		<cfquery name="AllEmployers" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from employers order by companyname
		</cfquery>
		...
		<cfif JobAction is "Edit">
			<cfquery name="Jobs" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
		...
		<cfif isDefined('form.submit')>
			...
			<cfif form.PayByInvoice is "no"><cfset form.PayByInvoice=0><cfelse><cfset form.PayByInvoice=1></cfif>
			<cfif form.Status eq "active"><cfset form.Status=1></cfif>
			<cfif form.Status neq "1"><cfset form.Status=0></cfif>
			<cfif #JobAction# is "Update">
				...
				<cfquery name="update" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
					...
					JobStarts='#form.JobStarts#',
					JobEnds='#form.JobEnds#',
					HotJob='#form.HotJob#',
					...
					JobPay='#form.jobPay#',
					PayByInvoice='#form.PayByInvoice#',
					Status='#form.Status#',
					employerID='#form.EmployerID#',
					JobDetails='#form.JobDetails#',
					JobPayPer='#form.JobPayPer#'
					where jobid=#jobid#
				</cfquery>
			<cfelse>
		...
		<cfquery name="AllJobs" datasource="#application.dsn#" password="#application.dsnpword#" username="#application.dsnuname#">
			select * from Jobs 
		...
		<TABLE>
			...
			<td><select name="Status">
					<option value="1" <cfif #Status# is "Active" or #Status# is 1>selected</cfif>>Active</option>
					<option value="0" <cfif #Status# is "Pending" or #status# is 0>selected</cfif>>Pending</option>
			</select>
			</td>
			</tr>
			...
			<td><select name="StateID" size="5" multiple>
				<cfset xStateID=replace(tStateID,"~",",","ALL")>
				<cfloop query="AllStates">
					<option value="#AllStates.StateID#" <cfif #ListFind(xStateID,AllStates.StateID)#>selected</cfif>>#stateName#</option>
				</cfloop>
			...
			<td valign=top> Pay By Invoice</td>
			<td><select name="PayByInvoice">
					<option value="1"<cfif #trim(tPayByInvoice)# is "1" or #tPayByInvoice# is 1> selected</cfif>>Yes</option>
					<option value="0"<cfif #trim(tPayByInvoice)# is "0" or #tPayByInvoice# is 0> selected</cfif>>No</option>
			</select>
			</td>
			</tr>
		...
		<td><p><Cfif #getEmployer.recordcount# gt 0>#getEmployer.companyname#<cfelse>#employerid#</Cfif></p></td>
	...
	<cffunction access="remote" name="JobCategoryAdmin" output="true" returntype="string">
		...
		<TABLE>
			...
			<tr>
			<td valign=top> Select Job Type?</td>
			<td>
				<select name="Internship">
					<option value="0">General</option>
					<cfloop query="jobTypes">
						<option value="#jobTypes.typeID#"<cfif #jobtypes.typeID# is #internship#> selected</cfif>>#jobTypes.TypeName#</option>
					</cfloop>
				</select>
				</td>
			</tr>
