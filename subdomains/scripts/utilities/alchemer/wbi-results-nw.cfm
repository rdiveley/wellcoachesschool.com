...
<cfif !isDefined('getMind_1')>
    ...
    <cfchart format="html" 
        ...
        <cfset colorList = ["##ffffff", "##f40090", "##25214e", "##0ae5ed", "##a6a6a6"] />
        <cfset colorIndex = 1 />
    ...
    <cfchart format="html" 
        ...
        <cfloop query="getWork_1">
            <cfif listFindNoCase(local.workTop5, evaluate('getWork_1.question'), "|")>
                <cfchartseries type="curve" serieslabel="#evaluate('getWork_1.question')#" markerstyle="circle" color="#colorList[colorIndex]#">
                    <cfloop from="1" to="#structCount(local.results)#" index="local.count2">
                        <!--- Ensure the structure exists before accessing --->
                        <cfif structKeyExists(local.work, local.count2) AND structKeyExists(local.work[local.count2], getWork_1.id)>
