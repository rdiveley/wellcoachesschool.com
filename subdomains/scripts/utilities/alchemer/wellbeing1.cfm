...
<!--- CFDUMP: Debugging by rdiveley --->
  ...
  <cfquery name="getAverageMind" dbtype="query">
       select answer
       from myquery
       where section_id = 1 and id not in (85,87,88,89,90)
   </cfquery>
   ...
   <cfset averageStruct.Mind = val(section_mind_average) />
    ...
    <cfquery name="getAverageBody" dbtype="query">
       select answer
       from myquery
       where section_id = 3 and id not in (85,87,88,89,90)
   </cfquery>
   ...
   <cfquery name="getAverageWork" dbtype="query">
       select answer
       from myquery
       where section_id = 4 and id not in (85,87,88,89,90)
   </cfquery>
   ...
   <cfquery name="getAverageLife" dbtype="query">
       select answer
       from myquery
       where section_id = 5 and id not in (85,87,88,89,90)
   </cfquery>
...
#trim(local.email)#
   <cfmail to="rdiveley@wellcoaches.com"  subject="Your Well-being Inventory" from="wellcoaches@wellcoaches.com" type="html" >
            #results#
    </cfmail> 
