...
<cfset columnList = "datesubmitted,id,lesson,email,surveyTitle,hours,score,premiumHours,originaldate" />
	...
	<!--- answer key --->
	  <cfset SGurl = "https://restapi.surveygizmo.com/head/survey/2955724/surveyresponse">
		...
		<cfset answerStruct["[question(218)]"] = "a. requires being aware of another person's suffering, understanding it, having feelings about it and taking action to relieve it."/>
		<cfset answerStruct["[question(219)]"] = "a. The coach having a different agenda than the client"/>
		<cfset answerStruct["[question(221)]"] = "d. it is important to remember that the client is not the device"/>
		<cfset answerStruct["[question(222)]"] = "b. a federal law that protects individuals' personal health information"/>
		
 	   <cfset SGurl = "https://restapi.surveygizmo.com/head/survey/8427958/surveyresponse">
  ...
  <cfloop list="#masterListID#" index="id" delimiters="%%">
    <cfset qid   = listFirst(id,'|')>
    <cfset label = listLast(id,'|')>
    <cfset keyKey = "[question(#qid#)]">

    <!-- safe lookups -->
    <cfset keyAns     = structKeyExists(answerStruct, keyKey)  ? answerStruct[keyKey]  : "">
    <cfset studentAns = structKeyExists(studentStruct, keyKey) ? studentStruct[keyKey] : "">

    <!-- only compare when both sides are non-empty -->
    <cfif len(keyAns) AND len(studentAns)
          AND compareNoCase(listFirst(keyAns,'.'), listFirst(studentAns,'.')) NEQ 0>
      <tr>
