...
<cfif isDate(renewedDate) AND isDate(REcertEndDate)>
   <h2> Hours are for Certification period: #renewedDate# to #REcertEndDate# </h2>
</cfif>
<!---<cftry>--->
  <cfset columnList = "datesubmitted,id,lesson,email,surveyTitle,hours,score,premiumHours" />

  <cfset SurveyList = QueryNew("#columnList#","#REReplace(RepeatString('varchar,',listLen(columnList)), ",+$", "")#") />
  <cfset allsurveys = "1013764,1020531,1120644,1060665,1330743,1117522,1026874,1117550,1447572,c,1849174,1953823,1994464,1382009,2338773,1959806,1144369">
         		...
         		<tr>
					...
					<cfif !listFind(NoHours,surveyTitle)>
						...
						<cfset total = total + 10>
                            <cfelseif surveyTitle eq 1447572 OR surveyTitle eq 1144369 OR surveyTitle eq 1959806>
					...
					<cfif !listFind(NoHours,surveyTitle)>
						...
						<cfset total = total + 10>
						  ...
						  <cfelseif surveyTitle eq 1026874 AND datePart('yyyy',datesubmitted) LTE '2015'>
 								 #iif(len(hours),hours,0)#
                                 <cfset total = total + iif(len(hours),hours,0)>
                          <cfelseif surveyTitle eq 1447572 OR surveyTitle eq 1959806 OR surveyTitle eq 1144369>
                                  #iif(len(hours),hours,0)#
                                 <cfset total = total + iif(len(hours),hours,0)>
