...
<cfset columnList = "datesubmitted,id,lesson,email,surveyTitle,hours,score,premiumHours,originaldate" />
	...
	<!--- answer key --->
	  <cfset SGurl = "https://restapi.surveygizmo.com/head/survey/2955724/surveyresponse">
 	   ...
 	   <cfset SGurl = "https://restapi.surveygizmo.com/head/survey/3137354/surveyresponse">
	  ...
	  <cfset studentStruct = studentData.data[1]>
		...
		<cfloop list="#masterListID#" index="id" delimiters="%%">
			<cfif (structKeyExists(answerStruct,'[question(#listFirst(id,'|')#)]') and len(answerStruct['[question(#listFirst(id,'|')#)]']))
				AND (structKeyExists(studentStruct,'[question(#listFirst(id,'|')#)]') AND len(studentStruct['[question(#listFirst(id,'|')#)]']))
				AND compareNoCase(listFirst(answerStruct['[question(#listFirst(id,'|')#)]'],'.'),listFirst(studentStruct['[question(#listFirst(id,'|')#)]'],'.')) NEQ 0>
				<tr>
